...
class Endpoint_Model(nn.Module):
    ...
    def forward(self, reference_images, reference_components, reference_strokes, target_images,
        ...
        patch_reference, patch_component_reference, patch_stroke_reference, patch_target_ori, patch_target_trans, \
        ...
        return patch_reference, patch_component_reference, patch_stroke_reference, patch_target_ori, patch_target_trans, \
            endpoint_offset, endpoint_offset_trans, endpoint_gt_offset_trans
    ...
    def get_points_and_raster_image(self, reference_images, reference_components, reference_strokes, target_images,
        ...
        curr_patch_image_tar_ori_out = torch.squeeze(curr_patch_image_tar_temp, dim=-1)  # (N, raster_size, raster_size), [0.0-stroke, 1.0-BG]
        ...
        curr_patch_image_tar_temp = cropped_outputs[:, :, :, 0:1]  # (N, raster_size, raster_size, 1), [0.0-stroke, 1.0-BG]
        curr_patch_image_tar_trans_out = torch.squeeze(curr_patch_image_tar_temp, dim=-1)  # (N, raster_size, raster_size), [0.0-stroke, 1.0-BG]

        curr_patch_image_tar_trans = normalize_image_m1to1(curr_patch_image_tar_temp)
        # (N, raster_size, raster_size, 1), [-1.0-stroke, 1.0-BG]

        encoded_z = self.build_encoder_endpoint(curr_patch_image_ref, curr_patch_stroke_ref, curr_patch_image_tar_trans)  # (N, raster_size, raster_size, 1), [0.0-stroke, 1.0-BG]
        ...
        endpoint_offset = spatial_transform_reverse_point_with_additional(
            endpoint_offset_trans.unsqueeze(dim=1), curr_window_size, cursor_position_loop_tar * float(self.image_size),
            ...
            additional_offset, additional_scale
        )  # (N, 1, 2), [-1.0, 1.0]
        ...
        endpoint_gt_offset_trans = spatial_transform_point_with_additional(
            endpoint_offset_gt, curr_window_size, cursor_position_loop_tar * float(self.image_size),
            target_transform_cursors * float(self.image_size), target_transform_win_sizes,
            target_transform_angles, target_transform_shear_x_angles, target_transform_shear_y_angles,
            additional_offset, additional_scale)  # (N, 1, 2), [-1.0, 1.0]
        ...
        return curr_patch_image_ref_out, curr_patch_component_ref_out, curr_patch_stroke_ref_out, \
               curr_patch_image_tar_ori_out, curr_patch_image_tar_trans_out,  \
               endpoint_offset, endpoint_offset_trans, endpoint_gt_offset_trans
...
class FullModel(object):
    ...
    def train_step(self, step, data_set, vec_loss_mean):
        ...
        _, _, _, _, endpoints_offset_pred, _, _ = \
    ...
    def save_log_images(self, data_set, save_root, step_num, save_num=20):
        ...
        with torch.no_grad():
            for batch_i in range(batch_num):
                ...
                patch_reference, patch_stroke_reference, patch_target_ori, patch_target_trans, \
                ...
                patch_target_trans_np = patch_target_trans.cpu().data.numpy()
                ...
                for p_i in range(patch_reference_np.shape[0]):
                    ...
                    patch_target_trans_np_i = save_image(patch_target_trans_np[p_i], save_root, 'tar_trans-' + str(img_index) + '-' + endpoint_id + '.png')

                    save_image_overlap(patch_target_trans_np[p_i], patch_reference_np_i, save_root,
                                       'tar_trans_vis-' + str(img_index) + '-' + endpoint_id + '.png')
        ...
        with (torch.no_grad()):
            for batch_i in range(batch_num):
                ...
                for chunk_i in range(chunks):
                    _, _, _, _, endpoints_offset_pred_ch, _, _ = \
            ...
            assert len(point_error_set_local) == len(point_error_set_global) == total_endpoint_num
        ...
        with torch.no_grad():
            for batch_idx_offset in batch_idx_offsets:
                ...
                for batch_i in range(batch_num):
                    ...
                    patch_target_ori_np = []
                    patch_target_trans_np = []
                    endpoints_offset_pred_np = []
                    endpoints_offset_pred_trans_np = []
                    endpoints_offset_gt_trans_np = []

                    for chunk_i in range(chunks):
                        patch_reference_ch, patch_component_reference_ch, patch_stroke_reference_ch, patch_target_ori_ch, patch_target_trans_ch, \
                        ...
                        patch_target_trans_ch_ch = patch_target_trans_ch.cpu().data.numpy()
                        ...
                        patch_target_trans_np.append(patch_target_trans_ch_ch)
                    ...
                    patch_target_trans_np = np.concatenate(patch_target_trans_np, axis=0)
                    ...
                    for p_i in range(patch_reference_np.shape[0]):
                        ...
                        save_image_overlap(patch_target_trans_np[p_i], patch_reference_np_i, save_root,
                                           'tar_trans_vis-' + str(img_index) + '-' + endpoint_id + '.png')
                        save_image_overlap(patch_target_trans_np[p_i], patch_component_reference_np_i, save_root,
                                           'tar_trans_comp_vis-' + str(img_index) + '-' + endpoint_id + '.png')
                        save_image_overlap(patch_stroke_reference_np[p_i], patch_target_trans_np_i, save_root,
                                           'ref_stroke_vis-' + str(img_index) + '-' + endpoint_id + '.png')
