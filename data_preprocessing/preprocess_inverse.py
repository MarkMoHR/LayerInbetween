...
def find_connected_points(database, connect_threshold, process_img_ids=None):
    ...
    for fi, filename in enumerate(all_files):
        ...
        params_path_tar1 = os.path.join(vector_data_dir, str(img_index) + '_ref.jsonl')
        ...
        params_path_tar0 = os.path.join(data_base_extra, 'params', 'tar_pred-' + str(img_index) + '.jsonl')
        ...
        if True:  # vis
            ...
            vis_path = os.path.join(save_base_vis, str(img_index) + "_ref.png")
...
def curve_grouping(database, example_info_map, line_thickness, process_img_ids=None):
    ...
    for fi, filename in enumerate(all_files):
        ...
        group_labels, optical_flow_dir = curve_grouping_flow_inv(database, stroke_data_b_tar0, stroke_data_b_tar1_raw, curves_endpoint_connected_state_raw,
                                                                 image_size, img_index,
                                                                 example_info_map[str(img_index)]["optical_flow_method"],
                                                                 example_info_map[str(img_index)]["use_distance_transform"])
...
def gen_stroke_images(database, line_thickness, process_img_ids=None):
    ...
    for fi, filename in enumerate(all_files):
        ...
        params_path_tar1 = os.path.join(vector_data_dir, filename)
        with open(params_path_tar1, "r+") as f:
            for item in jsonlines.Reader(f):
...
if __name__ == '__main__':
    ...
    load_svg(os.path.join(database, '[0inv]'), line_thickness, process_img_ids)

    vector_curve_splitting(os.path.join(database, '[0inv]'), line_thickness, process_img_ids)

    find_connected_points(os.path.join(database, '[0inv]'), connect_threshold, process_img_ids)

    curve_grouping(os.path.join(database, '[0inv]'), example_info_map, line_thickness, process_img_ids)

    ## For component transform task
    component_translation_with_optical_flow_main(os.path.join(database, '[0inv]'), line_thickness, None, None, process_img_ids,
                                                 data_extra=example_info_map)
    gen_component_images(os.path.join(database, '[0inv]'), line_thickness, process_img_ids, data_extra=example_info_map)

    ## For endpoint matching task
    gen_stroke_images(os.path.join(database, '[0inv]'), line_thickness, process_img_ids)

    ## For control point matching task
    gen_stroke_images_ctrlpoint(os.path.join(database, '[0inv]'), line_thickness, process_img_ids)