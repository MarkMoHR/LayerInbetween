...
def load_svg(database, line_thickness, process_img_id, img_size_default=768):
...
def gen_stroke_chain_images(database, line_thickness, process_img_id):
    ...
    vector_data_path = os.path.join(vector_data_dir, str(process_img_id) + '_ref.jsonl')
...
def vector_curve_splitting(database, line_thickness, process_img_id):
    ...
    img_file_path = os.path.join(img_dir, str(process_img_id) + '_ref.png')
    img = Image.open(img_file_path).convert('RGB')
    img_width, img_height = img.width, img.height
    assert img_width == img_height
    ...
    vector_vis_path = os.path.join(save_base_color_separate, str(process_img_id) + "_ref.png")
    draw_sketch_stroke_cairo([stroke_data_b_split], vector_vis_path, is_bezier=True,
                                side=img_width, line_diameter=line_thickness)

    vector_vis_path = os.path.join(save_base_color_separate_chain, str(process_img_id) + "_ref.png")
    stroke_data_new_b_ = [[item] for item in stroke_data_b_split]
    draw_sketch_stroke_chain_cairo(stroke_data_new_b_, vector_vis_path, is_bezier=True,
                                    side=img_width, line_diameter=line_thickness)


def curve_grouping(database, line_thickness, optical_flow_method, use_distance_transform, should_merge_single_curve, process_img_id):
    ...
    # curve_grouping_depth(database, stroke_data_b_raw, process_img_id)
    group_labels, optical_flow_dir = curve_grouping_flow(database, stroke_data_b_raw, image_size, process_img_id,
...
def curve_grouping_vis(database, line_thickness, optical_flow_method, use_distance_transform, process_img_id):
...
def curve_translation_with_optical_flow_main(database, line_thickness, optical_flow_method, use_distance_transform, process_img_id):
    print('============== curve_translation_with_optical_flow_main ==============')
    curve_translation_with_optical_flow(database, line_thickness, optical_flow_method, use_distance_transform, process_img_id)


def component_translation_with_optical_flow_main(database, line_thickness, optical_flow_method, use_distance_transform, process_img_id,
                                                 data_extra=None):
    print('============== component_translation_with_optical_flow_main ==============')
    component_translation_with_optical_flow(database, True, line_thickness, optical_flow_method, use_distance_transform, process_img_id,
                                            data_extra=data_extra, gen_time=gen_time)


def gen_component_images(database, line_thickness, process_img_id, data_extra=None):
    ...
    component_image_dir = os.path.join(component_image_base, str(process_img_id))
    ...
    if data_extra is not None:
        ...
        vector_data_path_extra = os.path.join(data_base_extra, 'params', 'tar_pred-' + str(process_img_id) + '.jsonl')
...
def gen_stroke_images_ctrlpoint(database, line_thickness, process_img_id):
...
if __name__ == '__main__':
    ...
    if '-Gen' not in database:
        load_svg(database, line_thickness, process_img_id)

        vector_curve_splitting(database, line_thickness, process_img_id)
        # gen_stroke_chain_images(database, line_thickness, process_img_id)
        # curve_translation_with_optical_flow_main(database, line_thickness, optical_flow_method, use_distance_transform, process_img_id)

        curve_grouping(database, line_thickness, optical_flow_method, use_distance_transform, should_merge_single_curve, process_img_id)
    else:
        curve_grouping_vis(database, line_thickness, optical_flow_method, use_distance_transform, process_img_id)

    ## For component transform task
    component_translation_with_optical_flow_main(database, line_thickness, optical_flow_method, use_distance_transform, process_img_id)
    gen_component_images(database, line_thickness, process_img_id)

    ## For endpoint matching task
    gen_stroke_images(database, line_thickness, process_img_id)

    ## For control point matching task
    gen_stroke_images_ctrlpoint(database, line_thickness, process_img_id)