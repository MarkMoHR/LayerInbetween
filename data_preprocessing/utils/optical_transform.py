...
def component_translation_with_optical_flow(data_base, do_real, line_thickness, optical_flow_method, use_distance_transform, process_img_ids,
    ...
    for fi, filename in enumerate(all_files):
        ...
        if data_extra is not None:
            optical_flow_method = data_extra[str(img_index)]["optical_flow_method"]
            use_distance_transform = data_extra[str(img_index)]["use_distance_transform"]
        else:
        ...
        vector_data_path_ref = os.path.join(vector_data_dir, filename)
        ...
        if data_extra is not None:
            ...
            vector_data_path_ref_extra = os.path.join(data_base_extra, 'params', 'tar_pred-' + str(img_index) + '.jsonl')
        ...
        # print('### Processing image', fi, '/', len(all_files), ':', img_index)

        if not do_real:
            ref_img_path = os.path.join(data_base, 'raster_black', 'sketch_' + str(img_index) + '_bezier-ref.png')
            tar_img_path = os.path.join(data_base, 'raster_black', 'sketch_' + str(img_index) + '_bezier-tar.png')
        else:
            ref_img_path = os.path.join(data_base, 'raster_black', str(img_index) + '_ref.png')
            tar_img_path = os.path.join(data_base, 'raster_black', str(img_index) + '_tar.png')
        ...
        translated_params_save_path = os.path.join(translated_param_save_based, str(img_index) + '.jsonl')
        ...
        save_path = os.path.join(translated_save_based, 'sketch_' + str(img_index) + '.png')
        warped_img_bg_png.save(save_path, 'PNG')


def curve_translation_with_optical_flow(data_base, line_thickness, optical_flow_method, use_distance_transform, process_img_ids):
    black_threshold = 200
